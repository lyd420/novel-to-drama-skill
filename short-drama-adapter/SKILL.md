...
在生成分集前，**必须确认输入文件符合以下标准**：

**如果发现以下问题，立即停止并返回 STEP 2**：
- ❌ 发现中文字符 → 返回重新翻译
- ❌ 发现拼音姓名（如 Xiao Nian, Jing Xing）→ 返回重新命名
- ❌ 发现中文地名（如 Beijing, Shanghai）→ 返回重新本地化

### 输出要求

分集脚本必须使用**纯英文**（基于已本地化的英文版）：
- 人名使用英文全名（Stella Gray，不是 Gu Xiao Nian）
- 地名使用英文地名（Washington D.C.，不是 Beijing）
- 无中文标点、无中文注释
