...
### 第四步：姓氏选择与家族命名体系（含去重检查）

**核心原则**：每个中文姓氏对应一个英文姓氏，家族成员共享同一姓氏，**且不与历史小说重复**。
...
该文件记录了所有历史小说使用过的：
- 家族姓氏（Gray, Foster, Shaw等）
- 人物全名（Stella Gray, Ethan Foster等）
- 主要地名（Washington D.C., Manhattan等）
...
3. **主要地名** - 如果"Manhattan, NY"已被使用，建议选择其他城市

**可重复使用**：
- 通用名字（Stella, Ethan等）+ 不同姓氏
- 同一城市不同类型地点（Manhattan作为商业区 vs Upper East Side作为住所）
...
历史记录：Washington D.C.（已被使用）
处理：
  - 同一地点不同故事：可以继续使用（读者不会混淆）
  - 不同地点：选择替代城市
    - 政治中心备选：Arlington, VA / Alexandria, VA
    - 或选择其他类型城市：Boston, Chicago
```
...
**同时更新**：`references/used-names.md` - 记录本次使用的家族姓氏、人名和地名
...
- **历史术语表**：[references/used-names.md](references/used-names.md) - 已使用的英文名、家族姓氏和地名记录
