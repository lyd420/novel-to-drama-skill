...
**翻译完成后，输出中必须完全不存在任何中文字符**，包括：

- ❌ 禁止保留：章节标题如"第一章" → 改为 "Chapter 1"
- ❌ 禁止保留：人名旁边的拼音标注
- ❌ 禁止保留：中文引号「」、顿号、书名号《》
- ❌ 禁止保留：中文单位如"元"、"斤"、"里"
- ❌ 禁止保留：中文语气词、网络用语
...
| 错误做法 | 正确做法 |
|----------|----------|
| ❌ Xiao Nian Gu | ✅ Stella Gray |
| ❌ Gu Xiao Nian | ✅ Stella Gray |
| ❌ Jing Xing Fu | ✅ Ethan Foster |
| ❌ Mr. Gu | ✅ Mr. Gray |
| ❌ Miss Fu | ✅ Ms. Foster |

**强制要求**：
1. **名字必须是标准英文名**（Stella, Ethan, Alexander 等），不是拼音
2. **姓氏必须是英文姓氏**（Gray, Foster, Shaw 等），不是拼音
3. **称呼必须用英文姓氏**：Mr. Gray（不是 Mr. Gu），Ms. Foster（不是 Miss Fu）
4. **公司名必须用英文姓氏**：Gray Corporation（不是 Gu Corporation）
...
【姓名格式检查】
- 确认所有人名格式为：First Last（如 Stella Gray）
- 确认无拼音残留：不含 Xiao, Nian, Jing, Xing 等拼音
- 确认称呼正确：Mr./Ms./Mrs. + 英文姓氏

【地名检查】
- 确认无中文地名：不含 Beijing, Shanghai 等拼音
- 确认格式正确：Washington, D.C. / New York, NY
```
